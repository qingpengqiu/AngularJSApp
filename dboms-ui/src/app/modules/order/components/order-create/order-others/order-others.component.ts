...
import {
  MaterialInfo, SaleOrderForm, ChannelType, CurrencyType, InvoiceType, OrderType, Platform, Rebatetype, LoadingInfo, UnadingInfo, TermInfo, AccoriesInfo, SaleOrderInfo, CurrentUserInfo,
  ...
  */
  getFormDataCallBack(data) {
    ...
    if (data["DeliveryData"]) {
      ...
      this.formData["DeliveryData"] = []
    }
    let currentUser = new CurrentUserInfo();
    let user = this.userInfo;
    let saleOrderInfo = new SaleOrderInfo();
    if (data["SalesOrderData"]) {
      saleOrderInfo = data["SalesOrderData"];
    }
    if (data["CurrentUser"]) {
      currentUser = data["CurrentUser"];
      saleOrderInfo.CustomerERPCode = "A";
      saleOrderInfo.CustomerName = "一次性售达方";
      saleOrderInfo.Receivable = "0.00";
      saleOrderInfo.Overdue = "0.00";
      ...
      // saleOrderInfo.SalesOrganizationID = currentUser.CompanyCode.slice(2) + "02";
      // saleOrderInfo.WFApproveUserJSON = JSON.stringify(data["WFApproveUserJSON"]);
    };
    ...
    //调取，最大返款率是否判断接口
    if (this.formData.SalesOrderData["YWFWDM"]) {
  ...
  */
  shipToCallBack(data?, i?) {
    if (data && data == "del") {
      ...
      let _thisMaterial = data["info"];
      if (acceptMaterialList && acceptMaterialList.length > 0) {
        let hasSameMaterial = false;
        acceptMaterialList.forEach(function(item, index) {
          //物料编码，物料名称，物料批次全相同。为同一种物料
          if (_thisMaterial["MaterialCode"] === item["MaterialCode"] && _thisMaterial["MaterialName"] === item["MaterialName"] && _thisMaterial["Batch"] === item["Batch"]) {
            item["Quantity"] = item["Quantity"] + _thisMaterial["Quantity"];
            hasSameMaterial = true;
            return;
          }
        });
        if (!hasSameMaterial) {
          acceptMaterialList.push(_thisMaterial);
        }
      } else {
        acceptMaterialList.push(_thisMaterial);
      }
    ...
    if (this.formData.DeliveryData && this.formData.DeliveryData.length > 0) {
      ...
      if (this.formData.DeliveryData.length > 0) {
        ...
        deliveryData.forEach(function(item, index) {
          ...
          if (item.MaterialList) {
            item.MaterialList.forEach(function(m, i) {
              if (!m.MaterialCode || !m.MaterialName || !m.Factory || !m.Batch || !m.ConsignmentModeID || !m.StorageLocation) {
                windowService.alert({ message: "请正确编辑物料信息！", type: "fail" });
                needReturn = true;
                return;
              };
              if (m.Quantity == 0) {
