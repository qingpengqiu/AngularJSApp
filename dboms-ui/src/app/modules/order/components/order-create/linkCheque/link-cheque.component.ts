...
export class LinkChequeComponent implements OnInit {
  ...
  icheckFun(event) {
    ...
    if (this.chequeList.length > 0 && this.chequeList[0]["ReceiptType"] != this.IsRelateCheque) {
      let message = "";
  ...
  createInvoice(type?) {
    if (type == 1) {
      window.open(dbomsPath + "invoice/apply/invoice");
    } else if (type == 2) {
      window.open(dbomsPath + "invoice/apply/tradeticket");
    }
  }
}
