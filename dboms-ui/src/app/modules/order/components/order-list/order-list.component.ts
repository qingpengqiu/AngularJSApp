...
export class OrderListComponent implements OnInit {
  ...
  initData() {
    ...
    if (this.currentMenuType == 'apply') {// apply/我的申请
      ...
      this.orderListService.getOrderListData(this.query).subscribe(
        data => {
          if (data.Result) {
  ...
  ngOnInit() {
    ...
    // //在初始化的时候 创建模型
    this.modalContract = this.xcModalService.createModal(SelectContractComponent);
  }
  //待我审批个数
  getUnApprove(type) {
    let params = {
  ...
  goToNewPage(SC_Code, salesOrderID, status) {
    ...
    if ((status === "0") || (status === "3")) {
      this.reload = true;
      // window.open(dbomsPath+'order/order-create?SO_Code='+salesOrderID+'&SC_Code='+SC_Code+'&type='+type);
      window.open(dbomsPath + 'order/order-create/' + type + '?SO_Code=' + salesOrderID + '&SC_Code=' + SC_Code);
      // window.open('/order/order-create?SO_Code='+salesOrderID);
    } else {
      //跳转查看页
      // window.open('/order/order-view?SO_Code='+salesOrderID);
  ...
  goToView(statUrl, taskState, salesOrderID) {
    ...
    if (taskState == '未处理') {//跳转审批页
      this.reload = true;
      let url = dbomsPath + 'order/order-view?' + statUrl.split('?')[1] + '&type=' + type;
      window.open(url);
      ...
      next: (v) => {
        if (v) {
          ...
          this.orderListService.deleteOrder(params).subscribe(data => {
            if (data.Result) {
              this.windowService.alert({ message: '删除成功', type: "fail" });
              this.currentTableData[parentIndex].SalesOrderList.splice(sonIndex, 1);
            } else {
              this.windowService.alert({ message: data.Message, type: "fail" });
            }
          })
