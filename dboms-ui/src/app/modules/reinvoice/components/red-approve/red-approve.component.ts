...
import { ApplyData, BaseDataSource, Invoice, Material, MaterialList, AccessItem } from "../../datamodel/redapply";
...
export class RedApproveComponent implements OnInit {
  ...
  hasUploadedFiles: any[] = [];//附件信息数据源 组件：{name: "a.txt", size: 5242880}
  ...
  ngOnInit() {
    ...
    this.nodeID = this.routerInfo.snapshot.queryParams['taskid'];
...
/**接口请求 begin */
    ...
    this.redApplyService.getApplyDataById(applyid).subscribe(
      data => {
        if (data.Result) {
            ...
            invoiceData.forEach(invoiceItem => {
              ...
              if (materialData && materialData.length > 0) {
                materialList.tableList = [].concat(materialData.filter((materialItem) => {
                  return materialItem.internalinvoiceno == internalinvoiceno;
                }));
              ...
              if (this.materialType == 0 || this.materialType == 1) {
                if (materialList.moneyTotalAmount == 0) return;
          ...
          if(this.applyData.apply.wfstatus == "驳回") this.getApproveHistory(this.applyId);
          //初始化当前提交按钮是否可点击
          if(this.applyData.apply.wfstatus == "提交") {
        ...
        if (data.Result) {
          ...
          if (this.isSubmited) {
            if (this.taskID) {
              ...
              this.windowService.alert({ message: "提交成功", type: "warn" });
              window.close();
            }
          } else {
            this.windowService.confirm({ message: "保存成功,是否关闭页面？" }).subscribe({
              next: (v) => {
                if (v) {
                  window.close();
                }
  ...
  getInvoiceByOrderNo(orderno) {
    ...
    if(this.selectedChildrenRevisetype.length == 0) {
...
/**页面功能 begin */
  ...
  pushInvoice(invoicedata) {
    ...
    if (getMaterialQuery.length > 0 && (this.materialType ==0 || this.materialType == 1 || this.materialType == 2)) {
  ...
  onClickFile(event){
    ...
    window.open(this.redApplyService.fileSerAddress + this.applyData.accessory[event.index].AccessoryURL);
  }
  //保存数据时  业务数据处理
  applyDataHandle() {
  ...
  applyDataValid(): boolean{
    ...
    let tempArr =[];
    this.materialList.forEach(item => {
      let isValid = item.tableList.some(this.materialListValid);
      if (!isValid) {
        let errorMsg = "冲退明细" + item.internalinvoiceno + "有必填项未填写";
  ...
  onSubmit(event){
    if (this.redApplyForm.valid && this.applyDataValid()) {//表单验证通过
      this.applyDataHandle();
      if (!this.taskID) this.applyData.apply.wfstatus = "提交";
      this.saveApply(this.applyData);
    }
  }
  //取消按钮事件
  ...
  //验证冲退明细列表必填项
  materialListValid = (item: Material, index) =>{
    let bool = true;
    switch (this.materialType) {
      case 0:
        bool = item.money && item.backmoney;
        break;
      case 1:
        bool = item.materialcode && item.num && item.factory && (item.money || item.money == 0) && (item.backmoney || item.backmoney == 0);
        break;
      case 2:
        bool = item.materialcode;
        break;
    }
    return bool;
  }
...
/** 冲退明细 begin */
  ...
  calculationTotal(tabitemi, item, i){
    ...
    //30%验证
    if(this.materialList[tabitemi].backMoneyTotalAmount >= this.materialList[tabitemi].moneyTotalAmount*0.3) {
