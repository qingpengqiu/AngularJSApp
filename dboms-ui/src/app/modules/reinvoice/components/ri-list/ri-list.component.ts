...
export class ReInvoiceComponent implements OnInit {
    ...
    public getMyList(e) {
        ...
        // this.liDoneList = false;
        this.approvePage = e;
        let headers = new Headers({ 'Content-Type': 'application/json', 'ticket': window.localStorage.getItem('ticket') });
        let options = new RequestOptions({ headers: headers });
    ...
    ngOnInit() {
        let localPerSon = window.window.localStorage.getItem('UserInfo');
    ...
    onCreat() {
        this.router.navigate(['/reinvoice/red-apply']);
    }
    onExamine(item) {
    ...
    onClickTrRouter(item) {
        ...
        if (item["wfstatus"] == "草稿" || item["wfstatus"] == "驳回") {
            if (item["taskTableURL"]) {
                window.open(item["taskTableURL"]);
            } else {
                window.open(serverUrl + "/reinvoice/red-apply?applyid=" + item["applyId"]);
            }
            ...
            if (item["taskTableURL"]) {
                ...
                window.open(serverUrl + "/reinvoice/red-approve?applyid=" + item["applyId"]);
            }
        }
    }
}
