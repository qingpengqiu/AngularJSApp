...
export class JyWfapprovalComponent implements OnInit {
    ...
    userArr: Array<any> = [{ id: '8a81e6c45cd4f5fe015cd4f7f3c91ecd', text: 'lichengg/李程' }, { id: '8a81e6c45cd4f5fe015cd4f7f3c91ecd', text: 'liusta/刘松涛' }];
    ...
    onApproval(approverType) {
        ...
        if (this.isGetData) {
            ...
            if (tempObj instanceof Object && !!tempObj.url) {
                this.http.post(tempObj.url, tempObj.params, options).subscribe(res => {
                    let data = res.json();
                    if (data.success) {
    ...
    _initApiappParms(approverType) {
        ...
        if (this.selectUsers.length != 0)
            appUserInfo = this.selectUsers[0].id;
