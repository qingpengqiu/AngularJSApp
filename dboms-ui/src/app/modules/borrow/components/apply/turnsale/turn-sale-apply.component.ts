...
export class TurnSaleApplyComponent implements OnInit {
    ...
    constructor(private http: Http, private route: ActivatedRoute, private location: Location, private router: Router, private windowService: WindowService, private xcModalService: XcModalService, private turnSaleService: TurnSaleService) { }
    ngOnInit() {
    ...
    queryReservation() {
        if (this.apply.reservationNo === undefined || this.apply.reservationNo === '') {
            this.windowService.alert({ message: "预留号不能为空", type: "success" });
            Object.keys(this.apply).forEach(key => this.apply[key] = "");
            this.unclearMaterialItemList = [];
        } else {
            this.turnSaleService.queryReservation(this.apply.reservationNo).then(data => {

                let poList: any = data.list;
                //console.log(poList);
                if (poList.length <= 0) {
                    this.windowService.alert({ message: "该预留号下没有未清项", type: "success" });
                    Object.keys(this.apply).forEach(key => this.apply[key] = "");
                    this.unclearMaterialItemList = [];
                } else {
                    this.unclearMaterialItemList = [];
                    for (let i = 0; i < poList.length; i++) {
                        let unClearItem: UnClearItem = poList[i].unClearItem;
                        this.unclearMaterialItemList = this.unclearMaterialItemList.concat(poList[i].unclearMaterialItemList);
                    }
                    //处理未清项物料列表为转销售物料列表，区别在于转销售的count=未清项物料的usableCount,转销售的totalAmount字段重新计算
                    this.unclearMaterialItemList.forEach(function (item) {
                        item.count = item.usableCount;//
    ...
    saveDraft() {
        ...
        this.turnSaleService.saveDraft(this.apply, this.unclearMaterialItemList).then(res => {
            if (res.success) {
                this.disabled = true;
    ...
    authenticateForm() {
        ...
        if (!this.myApplyForm.valid) {//表单验证未通过
            ...
            if (!valid) {
                flag = true;
                this.windowService.alert({ message: '交货单号只能输入数字', type: "fail" });
            } else {
                this.unclearMaterialItemList.forEach(item => {
                    if (item.count.toString().trim() === '') {
                        this.windowService.alert({ message: '数量不能为空', type: 'fail' });
                        flag = true;
                        return;
                    } else {
                        let reg = /^[1-9]\d*$/;
                        if (!reg.test(item.count.toString())) {
                            this.windowService.alert({ message: '数量必须为整数', type: 'fail' });
                            flag = true;
                            return;
                        }
                    }
    ...
    submitApply() {
        ...
        this.turnSaleService.submitForm(this.apply, this.unclearMaterialItemList).then(res => {
