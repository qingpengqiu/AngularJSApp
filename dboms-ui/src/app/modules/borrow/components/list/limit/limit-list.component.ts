...
export class LimitListComponent implements OnInit {
    ...
    search() {
        ...
        //console.log('22');
        this.limitListService.searchList(this.params, this.applyFlag).subscribe(res => {
            if (res.list) {
    ...
    export() {
        ...
        expParam.isMyApply = this.applyFlag === "1";
        expParam.isMyCheck = !expParam.isMyApply;
        expParam.isDept = this.params.isBaseStr === "1";
        if (expParam.isDept) {
            expParam.deptName = this.params.deptName;
        } else {
            expParam.baseName = this.params.deptName;
        }
        this.getDate(this.params.staDate, 'start');
        this.getDate(this.params.endDate, 'end');
