...
export class ContractQueryComponent implements OnInit {
  ...
  searchContrac(){
    this.http.post('/api/S_Contract/GetDeptSalesContract',{"BBName": this.department, "SYBName":"" })
          .map(res => res.json())
          .subscribe(res => {
              if(res.Data){
                if(this.contractParam !=""){//查询条件过滤
                  this.contractList = JSON.parse(res.Data).filter(
                      item => 
                        item.MainContractCode.indexOf(this.contractParam) >= 0 ||
                        item.SalesITCode.indexOf(this.contractParam) >= 0 ||
                        item.SalesName.indexOf(this.contractParam) >= 0 ||
                        item.SellerCompanyCode.indexOf(this.contractParam) >= 0 ||
                        item.SellerName.indexOf(this.contractParam) >= 0  
                    );
                }else{
