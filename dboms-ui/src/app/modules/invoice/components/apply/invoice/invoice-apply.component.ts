...
export class InvoiceApplyComponent implements OnInit {
    ...
    ngOnInit(): void{ 
        ...
        //草稿重新申请
        if(id !='-1'){
            this.invoiceApproveService.getInvoiceById(id).then(
              res =>{
    ...
    public initData(data:any) :void{
        this.invoiceApplyService.getLoginUser().then(res => {
            ...
            if(data =='-1'){
                ...
                this.invoiceInfo.platformName = res.sysUsers.flatName=="总部"?"北京":res.sysUsers.flatName;//平台名称
     ...
     * 查询欠款金额
     */
    toQueryDept(index,invoiceInfo:InvoiceInfo){
        if(invoiceInfo.customCode=="" || invoiceInfo.customCode== undefined){
            this.windowService.alert({ message: "请先选择客户", type: 'warn' });
            return;
        }
        if(invoiceInfo.payee=="" || invoiceInfo.payee== undefined){
            this.windowService.alert({ message: "请先选择收款人", type: 'warn' });
            return;
        }
        ...
        let date = {"index":index,"customCode":invoiceInfo.customCode,"customName":invoiceInfo.customName,
        "payee":invoiceInfo.payee,"payeeName":invoiceInfo.payeeName,"contractNum":invoiceInfo.contractNum,
    ...
    creatInvoice():void{
        ...
        if(this.invoices == [] || this.invoices.length==0){
            this.windowService.alert({ message: "请添加支票信息", type: 'warn' });
            return;
        }
        //检查必填项
        if(this.invoiceInfo.platformCode===undefined){
            this.windowService.alert({ message: "接收平台为必填", type: 'warn' });
            return;
        }

        //检查申请列表是否都保存
        this.invoices.forEach((inv,k) =>{
            if(inv.ticketNum===undefined){//必填验证
                this.windowService.alert({ message: "第"+(k+1)+"行支票号不能为空", type: 'warn' });
            }else if(!this.isDigital(inv.ticketNum)){
                 this.windowService.alert({ message: "第"+(k+1)+"行支票号必须为8位数字", type: 'warn' });
            }else if(inv.checkoutDate===undefined){
                this.windowService.alert({ message: "第"+(k+1)+"行出票日期不能为空", type: 'warn' });
            }else if(inv.ticketAmount===undefined){
                this.windowService.alert({ message: "第"+(k+1)+"行支票金额不能为空", type: 'warn' });
            }else if(!this.checkNumber(inv.ticketAmount)){
                this.windowService.alert({ message: "第"+(k+1)+"行支票金额必须为数字，可保留两位小数", type: 'warn' });
            }else if(inv.ticketAmount==0){
                this.windowService.alert({ message: "第"+(k+1)+"行支票金额不能为0", type: 'warn' });
            }else if(inv.checkoutAccount===undefined){
                this.windowService.alert({ message: "第"+(k+1)+"行出票人账号不能为空", type: 'warn' });
            }else if(inv.checkoutBank===undefined){
                this.windowService.alert({ message: "第"+(k+1)+"行出票人银行不能为空", type: 'warn' });
            }else if(inv.payee===undefined){
                this.windowService.alert({ message: "第"+(k+1)+"行收款人不能为空", type: 'warn' });
                ...
                this.windowService.alert({ message: "第"+(k+1)+"行出票日期不能小于9天前", type: 'warn' });
            }
            return false;
        })
        ...
        this.windowService.confirm({message:'确定提交？'}).subscribe(r =>{
            if(r){
                 ...
                 this.invoiceApplyService.create(this.invoices)
                 .then(res => {
                     if(res.success){
                        this.windowService.alert({message:'操作成功',type:'success'});
                        setTimeout(function() {
                            window.opener.document.getElementById('invoiceQuery').click();
                            window.close();    
                        }, 1000);
                     }else{
    ...
    reCreatInvoice(): void{
        ...
        this.invoices.forEach((inv,k) =>{ 
            if(inv.ticketNum===undefined){//必填验证
                this.windowService.alert({ message: "第"+(k+1)+"行支票号空", type: 'warn' });
            }else if(!this.isDigital(inv.ticketNum)){
                 this.windowService.alert({ message: "第"+(k+1)+"行支票号必须为8位数字", type: 'warn' });
            }else if(inv.checkoutDate===undefined){
                this.windowService.alert({ message: "第"+(k+1)+"行出票日期空", type: 'warn' });
            }else if(inv.ticketAmount===undefined){
                this.windowService.alert({ message: "第"+(k+1)+"行支票金额空", type: 'warn' });
            }else if(!this.checkNumber(inv.ticketAmount)){
                this.windowService.alert({ message: "第"+(k+1)+"行支票金额必须为数字，可保留两位小数", type: 'warn' });
            }else if(inv.checkoutAccount===undefined){
                this.windowService.alert({ message: "第"+(k+1)+"行出票人账号空", type: 'warn' });
            }else if(inv.checkoutBank===undefined){
                this.windowService.alert({ message: "第"+(k+1)+"行出票人银行空", type: 'warn' });
            }else if(inv.payee===undefined){
                this.windowService.alert({ message: "第"+(k+1)+"行收款人空", type: 'warn' });
            }else if(inv.customCode===undefined){
                this.windowService.alert({ message: "第"+(k+1)+"行客户代码空", type: 'warn' });
            }else if(inv.customName===undefined){
                this.windowService.alert({ message: "第"+(k+1)+"行客户名称空", type: 'warn' });
            }else if(inv.contractNum===undefined){
                this.windowService.alert({ message: "第"+(k+1)+"行合同编号空", type: 'warn' });
            }else if(inv.debtAmount !=undefined && !this.checkNumber(inv.debtAmount)){
                this.windowService.alert({ message: "第"+(k+1)+"行欠款金额必须为数字，可保留两位小数", type: 'warn' });
            }else if(!this.checkTheDate(inv.checkoutDate)){
                this.windowService.alert({ message: "第"+(k+1)+"出票日期不得小于9天之前", type: 'warn' });
            }
            return false;
        })
        ...
        this.windowService.confirm({message:'确定提交？'}).subscribe(r =>{
            if(r){
                 this.submitFlag = false;
                 this.invoiceApproveService.reapplyInvoice(this.invoices[0])
                 .then(res => {
                     if(res.success){
                        this.windowService.alert({message:'操作成功',type:'success'});
                        setTimeout(function() {
