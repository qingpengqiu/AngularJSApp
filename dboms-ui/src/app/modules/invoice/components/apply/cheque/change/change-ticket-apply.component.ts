...
export class ChangeTicketApplyComponent implements OnInit {
  ...
  constructor(private xcModalService: XcModalService, private windowService: WindowService, private changeService: InvoiceChangeService,private route: ActivatedRoute) {
  ...
  ngOnInit() {
     this.route.params.subscribe((params: Params) => {
            ...
            this.changeService.loadChangeInfo(x).subscribe(res => {
                ...
                // this.wfData.wfprogress = res.item[2];//流程图信息
                 ...
                 this.changeService.getApprovals(this.platformCode).subscribe(data=>{
    ...
    this.modalAddForm.onHide().subscribe(data => {
      this.changeList = this.changeList.filter(item => item.invoiceApplyId !== "-1");
      if (data) {
        data.forEach(item => {
          let changeInfo = new ChangeApplyInfo();
...
//下载换票模板
  ...
  searchInvoice(): void {
    ...
    let data = { "applyids": ids, "status": status,"typeval":"HP","sqruserItcode":this.itcode};
  ...
  save(): void {
    ...
    if (this.changeReson === "") {
      this.windowService.alert({ message: "请填写换票原因！", type: 'fail' });
      return;
    }
    if (!this.validCustom()) {//非同一客户
      this.windowService.alert({ message: "非同一个客户不能提交！", type: 'fail' });
      return;
    }
    ...
    this.changeList.forEach(item => {
      ...
      if (item.newTicketNumber === "" || item.newCheckoutDate === ""
        || item.newCheckoutAccount === "" || item.newCheckoutBank === ""
      ...
      if (ticketNumStr!==""&&ticketNumStr.indexOf(item.newTicketNumber)!==-1){
    ...
    if(isvalid){
       this.windowService.alert({ message: "必填项不能为空！", type: 'fail' });
       return;
    }
 
   if(checkNewTicketlen){
     this.windowService.alert({ message: "新支票号长度为8位数字!", type: 'fail' });
       return;
   }
   if(checkSameNewTicket){
       this.windowService.alert({ message: "新支票号不能相同！", type: 'fail' });
       return;
    }
    ...
    if(this.changeId!==""){
      ...
      if(isvalid){
        this.windowService.alert({ message: "新支票到期日最小为当日减9个自然日!！", type: 'fail' });
        return;
      }
    }
    ...
    this.changeService.submitChangeApply(data).then(res => {
      let rdata = res.json();
      if (rdata.success) {
        this.windowService.alert({ message: "换票申请发起成功!", type: 'success' });
        this.showbtn = false;
        window.opener.document.getElementById('searchChange').click(); 
      } else {
        this.showbtn = true;
        this.isClick = false;
  ...
  get validForm(): boolean {//校验提交按钮
    if (this.changeList.length <= 0) {
      return false;
    }
    return true;
  }
  validCustom(): boolean {//校验是否同一个客户
    let i = -1;
    let custCode = "";
    this.changeList.forEach(item => {
  ...
  getDate(date, invoice: ChangeApplyInfo) {
    ...
    if(this.dateDiff(dataObj,ninedate)>0){
      ...
      this.windowService.alert({ message: '最小日期为当日减9个自然日!', type: 'fail' });
      return;
    }
  ...
  dateDiff(d1,d2): number {//获取两个日期相差天数
  ...
  checkFloat(item) {
    if (!this.checkNumber(item.newTicketAmount)) {
      this.windowService.alert({ message: '请输入规范的数字，可保留两位小数', type: 'fail' });
      item.newTicketAmount = '';
      return false;
    }
  }
     ...
     if (data.success) {
         if (data.item.errorList.length > 0) {
                ...
                for (let i = 0; i < data2.length; i++) {
                    ...
                    changeInfo.payee = item.oldApplyId.payee;
                    changeInfo.payeeName=item.oldApplyId.payeeName;
                    changeInfo.customCode = item.oldApplyId.customCode;
                    changeInfo.customName = item.oldApplyId.customName;
                    changeInfo.newTicketNumber=item.newTicketNumber;
                    changeInfo.newCheckoutDate=this.getFormatDate(new Date(item.newCheckoutDate));
                    changeInfo.newCheckoutAccount=item.newCheckoutAccount;
                    changeInfo.newCheckoutBank = item.newCheckoutBank;
                    changeInfo.newTicketAmount=item.newTicketAmount;
                    this.changeList.push(changeInfo);
