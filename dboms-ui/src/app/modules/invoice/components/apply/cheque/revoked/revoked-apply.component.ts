...
export class RevokedApplyComponent implements OnInit {
  ...
  constructor(private windowService: WindowService, private xcModalService: XcModalService, private http: Http) {
    this.http.get(environment_java.server + 'common/getCurrentLoginUser', {})
      .map(res => res.json())
      .subscribe(data => {
        console.log(data.item.sysUsers);
        if (data.success) {
          var data1 = {
            userCN: data.item.sysUsers.userName, userEN: data.item.sysUsers.itcode.toLowerCase(), userID: data.item.sysUsers.itcode
          };
          this.sqr.push(data1);
          ...
          this.loadApprover(this.person1, "ChequeRiskApprove", "FLATCODE='" + this.platformCode + "'");//风险岗
          this.loadApprover(this.person2, "ChequeFinanceApprove", "FLATCODE='" + this.platformCode + "'");//设置财务审批人
  ...
  ngOnInit() {
    ...
    this.modalAddForm.onHide().subscribe((data) => {
      this.invoicelist = this.invoicelist.filter(item => item.id !== "-1");
      if (data) {
        data.forEach(item => {
  ...
  searchInvoice(): void {
    ...
    var data1 = { "applyids": ids, "status": "1,2,3,5,6,8,9", "typeval": "CP", "sqruserItcode": this.sqr[0].userEN };
  ...
  save(): void {
    ...
    if (this.sqr.length <= 0) {
      this.windowService.alert({ message: "申请人不能为空！", type: 'fail' });
      return;
    }
    ...
    if (this.revokedDept.length <= 0
      || this.revokedPhone.length <= 0 || this.revokedRemark.length <= 0) {
      this.windowService.alert({ message: "请填写必填项！", type: 'fail' });
      return;
    }
    ...
    if (this.person1.length <= 0) {
      this.windowService.alert({ message: "风险岗审批人不能为空！", type: 'fail' });
      return;
    }
    if (this.person2.length <= 0) {
      this.windowService.alert({ message: "财务岗审批人不能为空！", type: 'fail' });
      return;
    }
    var data = {
    ...
    console.log(data);
    this.http.post(environment_java.server + '/revoked/submit-revoked-applys', data, this.options)
      .toPromise()
      .then(res => {
        console.log(res);
        let rdata = res.json();
        ...
        if (rdata.success) {
          this.windowService.alert({ message: "撤票申请发起成功！", type: 'success' });
          this.showbtn = false;
        } else {
          this.windowService.alert({ message: rdata.message, type: 'fail' });
          this.isClick = false;
        }
  ...
  load_invoicelist = function (revokedId) {
    this.http.get(environment_java.server + '/revoked/invoice-applys')
      .map(res => res.json())
      .subscribe(res => {
        if (res.list) {
  ...
  loadChange() {
    window.location.href = '../../assets/downloadtpl/支票撤票-导入模板.xls';
  }
  changePerson(pinfo?) {
    ...
    if (pinfo&&pinfo.length>0) {
      ...
      this.http.get(environment_java.server + 'common/sys-people/' + pinfo[0]["userEN"], {})
        .map(res => res.json())
        .subscribe(data => {
  ...
  onFileCallBack(data) {//文件上传回传数据
    if (data.success) {
      if (typeof (data.item.error) != 'undefined' && data.item.error.length > 0) {
        this.windowService.alert({ message: data.item.error, type: 'fail' });
      } else {
        let data2 = JSON.parse(data.item.list);
        for (let i = 0; i < data2.length; i++) {
          ...
          if(!bol){
            ...
            this.windowService.alert({ message:"支票号：" +isnum+"已存在", type: 'fail' });
          }
  ...
  loadApprover(aid, functionCode, fields) {
    ...
    this.http.post(environment_java.server + 'revoked/invoice-approver', data1, this.options)
      .map(res => res.json()).subscribe(res => {
        var userli = res.item.key1;
        for (let i = 0; i < userli.length; i++) {
          let item1 = userli[i];
          var data1 = {
            userCN: item1.name, userEN: item1.itcode, userID: item1.id
          };
          aid.push(data1);
        }
      })
