...
import { InvoiceCommonListComponent } from "../../index";
declare var window;
...
export class TakebackApplyComponent implements OnInit {
  ...
  ngOnInit() { 
      this.activatedRouter.params.subscribe((params: Params) => {
        let id = params['id'];
        if(id !='-1'){
            this.invoiceTakebackService.getTakebackInfo(id).then(
              res =>{
    ...
    this.invoiceApplyService.getLoginUser()
       .then(user =>{
           ...
           this.itcodeAndName = user.sysUsers.itcode+"/"+user.sysUsers.userName;
  ...
  searchInvoice(): void {
    ...
    var data1 = { "applyids": ids, "status": "", "typeval": "QH", "sqruserItcode": this.person[0].userEN };
  ...
  takeback(){
      if(this.takebackRemark===""){
          this.windowService.alert({ message: "请填写取回原因", type: 'warn' });
          return;
      }
    ...
    this.windowService.confirm({message:'确定取回支票吗？'}).subscribe(r =>{
        if(r){
            ...
            this.invoiceTakebackService.takebackApply(idStr.substring(0,idStr.length-1),this.takebackRemark)
                .then(res =>{
                    if(res.success){
                        this.windowService.alert({message:'操作成功',type:'success'});
                        setTimeout(function() {
                            window.opener.document.getElementById('getTakebackApprove').click();
                            window.close();    
                        }, 1000); 
                    }else{
                        this.submitFlag="0"
  ...
  takeback2(){
    ...
    this.windowService.confirm({message:'确定重新发起取回申请吗？'}).subscribe(r =>{
        if(r){
            this.submitFlag="1";
            this.invoiceTakebackService.approveTakeback(this.invoicelist[0].id,"0",this.takebackRemark)
                .then(res =>{
                    if(res.success){
                        this.windowService.alert({message:'操作成功',type:'success'});
                        setTimeout(function() {
