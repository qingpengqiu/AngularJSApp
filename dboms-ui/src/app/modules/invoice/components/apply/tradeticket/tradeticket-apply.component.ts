   ...
   // payeeForm :XcModalRef;
    ...
    ngOnInit(): void{
        ...
        this.activatedRouter.params.subscribe((params: Params) => {
            ...
            //草稿重新申请
            if(id !='-1'){
                this.tradeTicketService.getTradeticketById(id).then(
                res =>{
                    this.tradeTickets.push(res.item[0]);
    ...
    public initData(data:any) :void{
        this.invoiceApplyService.getLoginUser().then(res => {
            ...
            if(data =='-1'){
                let plantformCode = res.sysUsers.flatCode.length===1?"0"+res.sysUsers.flatCode:res.sysUsers.flatCode;
                let aperson = {
                ...
                this.tradeTicketInfo.platformName = res.sysUsers.flatName=="总部"?"北京":res.sysUsers.flatName;//平台名称
                ...
                this.person.push(aperson);
                this.tradeTicketInfo.applyItcode = data.applyItcode;//申请人itcode
                this.tradeTicketInfo.applyUsername = data.applyUsername;//申请人姓名
                this.tradeTicketInfo.applyPhone = data.applyPhone;//申请人电话
                this.tradeTicketInfo.platformCode = data.platformCode;//平台code
                this.tradeTicketInfo.platformName = data.platformName;//平台名称
                this.tradeTicketInfo.applyDeptcode = data.applyDeptcode;
                this.tradeTicketInfo.applyDeptname = data.applyDeptname;
                this.tradeTicketInfo.businessScope = data.businessScope;//业务范围代码
                this.tradeTicketInfo.payee = data.payee;
                this.tradeTicketInfo.payer = data.payer;
                this.tradeTicketInfo.tradeNumber = data.tradeNumber;
                this.tradeTicketInfo.tradeAmount = data.tradeAmount;
                this.tradeTicketInfo.remark = data.remark;
                this.tradeTicketInfo.checkoutDate = data.checkoutDate;
     ...
     * 查询欠款金额
     */
    toQueryDept(index,tradeTicketInfo:TradeTicketInfo){
        if(tradeTicketInfo.customCode=="" || tradeTicketInfo.customCode== undefined){
        ...
        if(this.tradeTicketInfo.payee=="" || this.tradeTicketInfo.payee== undefined){
         ...
         if(tradeTicketInfo.contractNum=="" || tradeTicketInfo.contractNum== undefined){
            this.windowService.alert({ message: "请先选择合同编号", type: 'warn' });
            return;
        }
        let date = {"index":index,"customCode":tradeTicketInfo.customCode,"customName":tradeTicketInfo.customName,
        "payee":this.tradeTicketInfo.payee,"payeeName":this.tradeTicketInfo.payeeName,"contractNum":tradeTicketInfo.contractNum,
    ...
    creatInvoice(event):void{
        ...
        if(this.tradeTickets == [] || this.tradeTickets.length==0){
            this.windowService.alert({ message: "请添加商票信息", type: 'warn' });
            return;
        }
        ...
        this.tradeTickets.forEach((inv,k) =>{
            if(inv.customCode===undefined){
                this.windowService.alert({ message: "第"+(k+1)+"行客户代码空", type: 'warn' });
            }else if(inv.customName===undefined){
                this.windowService.alert({ message: "第"+(k+1)+"行客户名称空", type: 'warn' });
            }else if(inv.debtAmount !=undefined && !this.checkNumber(inv.debtAmount)){
                this.windowService.alert({ message: "第"+(k+1)+"行欠款金额必须为数字，可保留两位小数", type: 'warn' });
            }else if(inv.voucherAmount===undefined){
                this.windowService.alert({ message: "第"+(k+1)+"核销金额为空", type: 'warn' });
            }else if(inv.voucherAmount !=undefined && !this.checkNumber(inv.voucherAmount)){
                this.windowService.alert({ message: "第"+(k+1)+"行核销金额必须为数字，可保留两位小数", type: 'warn' });
            }
            if(inv.voucherAmount !=undefined && this.tradeTicketInfo.tradeAmount != undefined && inv.voucherAmount>this.tradeTicketInfo.tradeAmount){
                this.windowService.alert({ message: "第"+(k+1)+"行核销金额必须小于商票金额", type: 'warn' });
                 return false;
            }
            return false;
        }) 
       
        this.tradeTickets.forEach(
          tradeticket =>{
            tradeticket.applyItcode = this.tradeTicketInfo.applyItcode.toLocaleLowerCase();
            tradeticket.applyUsername = this.tradeTicketInfo.applyUsername;
            tradeticket.applyPhone = this.tradeTicketInfo.applyPhone;
            tradeticket.platformCode = this.tradeTicketInfo.platformCode;
            tradeticket.platformName = this.tradeTicketInfo.platformName;
            tradeticket.applyDeptname = this.tradeTicketInfo.applyDeptname;
            tradeticket.applyDeptcode = this.tradeTicketInfo.applyDeptcode;
            tradeticket.payee = this.tradeTicketInfo.payee;
            tradeticket.businessScope = this.tradeTicketInfo.businessScope;
            tradeticket.payer = this.tradeTicketInfo.payer;
            tradeticket.tradeNumber = this.tradeTicketInfo.tradeNumber;
            tradeticket.tradeAmount = this.tradeTicketInfo.tradeAmount;
            tradeticket.checkoutDate = this.tradeTicketInfo.checkoutDate;
            tradeticket.remark = this.tradeTicketInfo.remark;
          }
      )   
        var data = {
            "tradeTicketList": this.tradeTickets,
            "attachList": this.attachList
        }
        this.windowService.confirm({message:'确定提交？'}).subscribe(r =>{
            if(r){
                 this.submitFlag = false;
                 this.tradeTicketService.create(data)
                 .then(res => {
                     if(res.success){
                        this.windowService.alert({message:'操作成功',type:'success'});
                        setTimeout(function() {
                            window.opener.document.getElementById('tradeTicketQuery').click();
                            window.close();    
                        }, 1000);
                     }else{
    ...
    reCreatInvoice():void{
        ...
        if(this.tradeTicketInfo.tradeAmount !=undefined && !this.checkNumber(this.tradeTicketInfo.tradeAmount)){
                this.windowService.alert({ message: "商票金额必须为数字，可保留两位小数", type: 'warn' });
            return;
        }
        if(this.tradeTicketInfo.checkoutDate===undefined){
            this.windowService.alert({ message: "到期日期为必填", type: 'warn' });
        ...
        if(!this.checkTheDate(this.tradeTicketInfo.checkoutDate)){
            this.windowService.alert({ message: "到期日期不能小于9天前", type: 'fail' });
            return;
        }
        ...
        this.tradeTickets.forEach((inv,k) =>{
            if(inv.customCode===undefined){
                this.windowService.alert({ message: "第"+(k+1)+"行客户代码不能为空", type: 'warn' });
                 return false;
            }
            if(inv.customName===undefined){
                this.windowService.alert({ message: "第"+(k+1)+"行客户名称不能为空", type: 'warn' });
                 return false;
            }
            if(inv.debtAmount !=undefined && !this.checkNumber(inv.debtAmount)){
                this.windowService.alert({ message: "第"+(k+1)+"行欠款金额必须为数字，可保留两位小数", type: 'warn' });
                 return false;
            }
            if(inv.voucherAmount===undefined){
                this.windowService.alert({ message: "第"+(k+1)+"核销金额不能为空", type: 'warn' });
                 return false;
            }
            if(inv.voucherAmount !=undefined && !this.checkNumber(inv.voucherAmount)){
                this.windowService.alert({ message: "第"+(k+1)+"行核销金额必须为数字，可保留两位小数", type: 'warn' });
                 return false;
            }
            if(inv.voucherAmount !=undefined && this.tradeTicketInfo.tradeAmount != undefined && inv.voucherAmount>this.tradeTicketInfo.tradeAmount){
        ...
        this.windowService.confirm({message:'确定提交？'}).subscribe(r =>{
            if(r){
                 this.submitFlag = false;
                 this.tradeTicketService.reCreate(data)
                 .then(res => {
                     if(res.success){
                        this.windowService.alert({message:'操作成功',type:'success'});
                        setTimeout(function() {
