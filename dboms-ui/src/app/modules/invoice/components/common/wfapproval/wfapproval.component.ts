...
opinions: string = "";//审批意见
  ...
  userArr: Array<any> = [{ id: '8a81e6c45cd4f5fe015cd4f7f6072737', text: 'wuzk/邬正凯' }, { id: '8a81e6c45cd4f5fe015cd4f7f48121d7', text: 'liyuank/李媛' }];
  ...
  onApproval(approverType) {
    if (this.isGetData) {
      ...
      if (tempObj instanceof Object && !!tempObj.url) {
        ...
        this.http.post(tempObj.url, tempObj.params, options).subscribe(data => {
          if (data.json().success) {
           window.history.back();
          } else {
            this.WindowService.alert({ message: "审批出错:" + data.json().message, type: "fail" });
          }
          // this.smModal.show();
        });
      } else {
