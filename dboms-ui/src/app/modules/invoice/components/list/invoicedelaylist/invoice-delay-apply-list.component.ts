...
export class InvoiceDelayApplyListComponent implements OnInit {
    ...
    constructor(private windowService: WindowService, private router: Router, private http: Http) {
    ...
    goto(item) {
        ...
        if (this.tabType == "0" && this.applyPage == "0") {
            window.open("/invoice/delay/resubmit/" + id);
            // this.router.navigate(["/invoice/delay/resubmit/" + id]);            
        } else {
            this.router.navigate(["/invoice/delay/delayDetial/" + id]);
        }
