...
import { Query, InvoiceInfo, UserInfo, InvoiceQueryPo } from "../../apply/invoice/invoice-info";
...
export class InvoiceListComponent implements OnInit {
    ...
    ngOnInit() {  
         ...
         .then(res=>{
            ...
            if(data.success){
                ...
                //目前先所有角色都有销售角色权限（如果需要添加销售角色权限，则删除一下2行）
                this.flag = "0";
                this.applyPage = "0"  
            }
        }
    ...
    public getLoginUser(): void {
        this.invoiceApplyService.getLoginUser()
            .then(user => {
                let plantformCode = user.sysUsers.flatCode.length === 1 ? "0" + user.sysUsers.flatCode : user.sysUsers.flatCode;
                //初始化表单
    ...
    //点击页签事件
    changeapplytype(flowstates: string,clospanNum:number, flag: string): void {
        this.loading = true;
        ...
        this.invoiceApproveService.getApproveList(this.query).then(res => {
            ...
            if (res.list) {
                ...
                for (let i = 0; i < this.approveList.length; i++) {
                    ...
                    this.approveList[i].num = (apprData.pageNo - 1) * apprData.pageSize + (i + 1);
                    //添加延字
                    this.invoiceApplyService.getAllDelayInvoice().then(
                        
                        res =>{
                            if(res.item){
                                res.item.forEach(
                                    item => {
                                        if(item.invoiceNum == this.approveList[i].ticketNum && item.customCode == this.approveList[i].customCode){
    ...
    approve(f) {
        ...
        let idStr = "";
        this.approveList.filter(invoice => invoice.checked === true).forEach(invoice => {
            idStr += invoice.id + ",";
        });
        this.invoiceApproveService.approveInvoice(idStr.substring(0, idStr.length - 1), f)
            .then(res => {
                if (res.success) {
                    this.clickFlag = true;
    ...
    approve2(f) {
        ...
        this.approveList.filter(invoice => invoice.checked === true).forEach(invoice => {
            ...
            if (invoice.invoiceStatus == 5 || invoice.invoiceStatus == 6) {
                if (f == 1) {//商务人员取走
                    this.invoiceApproveService.approveInvoice(invoice.id, '6').then(
                        res => {
                            if (res.success) {
                                this.clickFlag = true;
                                this.fullCheckedIndeterminate = false;
                ...
                if (f == 2) {//申请人员取走
                    this.invoiceApproveService.approveInvoice(invoice.id, '7').then(
                        res => {
                            if (res.success) {
                                this.clickFlag = true;
                                this.fullCheckedIndeterminate = false;
            ...
            if (invoice.invoiceStatus == 8 || invoice.invoiceStatus == 9) {
                if (f == 1) {//商务人员取走
                    this.invoiceApproveService.approveInvoice(invoice.id, '9').then(
                        res => {
                            if (res.success) {
                                this.clickFlag = true;
                                this.fullCheckedIndeterminate = false;
                ...
                if (f == 2) {//申请人员取走
                    this.invoiceApproveService.approveInvoice(invoice.id, '10').then(
                        res => {
                            if (res.success) {
                                this.clickFlag = true;
                                this.fullCheckedIndeterminate = false;
    ...
    refuse(id: string, invoiceStatus: string) {
        ...
        this.windowService.confirm({ message: confirmStr }).subscribe(r => {
            if (r) {
                this.invoiceApproveService.approveInvoice(id, invoiceStatus)
                    .then(res => {
                        if (res.success) {
                            this.getApproveList();
                            this.getPayeeCountList();
    ...
    getPayeeCountList() {
        ...
        this.invoiceApproveService.getPayeeCountList(this.query)
            .then(resp => {
    ...
    // }
    getPayeeList(){
    this.http.post('/api/BaseData/GetCompanyInfo/',{"CompanyName": "","CompanyCode":""} )
          .map(res => res.json())
          .subscribe(res => {
              if(res.Data){
     ...
     */
    getBusinessList() {
        this.invoiceApproveService.getBusinessList()
        ...
        this.invoiceApplyService.getInvoiceList(this.query).then(res => {
            ...
            if (res.list) {
                ...
                for (let i = 0; i < this.invoiceList.length; i++) {
                    this.invoiceList[i].num = (pagerData.pageNo - 1) * pagerData.pageSize + (i + 1);
                    //获取状态名称
                    this.invoiceList[i].statusName = this.getStatusName(this.invoiceList[i].invoiceStatus);
                    //获取下一步审批人
                    if(this.invoiceList[i].invoiceStatus==0 || this.invoiceList[i].invoiceStatus==1 || this.invoiceList[i].invoiceStatus==6 || this.invoiceList[i].invoiceStatus==9){
                        this.getBussinApproves(this.invoiceList[i].platformCode).then(
                            res =>{
                                this.invoiceList[i].nextApprove = res.substring(0,res.length-1);
                            }   
                        );
                    }else if(this.invoiceList[i].invoiceStatus==2 || this.invoiceList[i].invoiceStatus==3 || this.invoiceList[i].invoiceStatus==5 || this.invoiceList[i].invoiceStatus==8){
                        this.getFinaceApproves(this.invoiceList[i].platformCode).then(
                            res =>{
                                this.invoiceList[i].nextApprove = res.substring(0,res.length-1);
                            }   
                        );
                    }else if(this.invoiceList[i].invoiceStatus==7 || this.invoiceList[i].invoiceStatus==10){
    ...
    export() {//EXCEL导出
        ...
        this.http.post(environment_java.server+"invoice/invoice-apply/exportExcel",
            this.queryPo, {
                responseType: 3
            })
            .map(res => res.json())
    ...
    goDetail(id):void{ 
        if(this.applyPage=='3' && this.flag=='0'){//重新申请
            window.open("/invoice/apply/invoice/"+id);
        }else if(this.applyPage=='0' && this.flag=='1'){//编辑
            window.open("/invoice/apply/invoiceEdit/"+id);
        }else{//查看详情
         window.open("/invoice/apply/invoicedetail/"+id); 
        } 
    }
