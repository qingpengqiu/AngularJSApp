...
export class ChangeTicketListComponent implements OnInit {
    ...
    constructor(private changeService:InvoiceChangeService,private router:Router) { }
    ...
    ngOnInit() {
         this.searchTermStream.debounceTime(500).distinctUntilChanged().map(searchTerm => {
    ...
    getChangeListDate() {
        ...
        this.stopQuery=this.changeService.getChangeListData(change).subscribe(data => {
             this.changeApplyFlowData = data.list;
             this.pagerData.set(data.pager);
             this.approveCount=data.approveCount;
             if(this.changeApplyFlowData&&this.changeApplyFlowData.length>0){
                 this.isshow=true;
             }else{
                 this.isshow=false;
             }
  ...
  goDetail(changeid):void{
      if(this.applyPage==="0"){
         window.open("/invoice/change/reapply/"+changeid);
      }else{
