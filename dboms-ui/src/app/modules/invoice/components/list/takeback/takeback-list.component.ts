...
export class TakebackListComponent implements OnInit {
    ...
    getTakebackApprove() {
        ...
        this.invoiceTakebackService.getTakebackApprove(this.peopleItcode,this.flag,this.approveStatus,pagerData.pageSize.toString(),pagerData.pageNo.toString()).then(
            res =>{
                 if (res.list) {
                    this.takebackList = res.list;
                    for (let i = 0; i < this.takebackList.length; i++) {
                        this.takebackList[i].num = (pagerData.pageNo - 1) * pagerData.pageSize + (i + 1);
                    }
                }
                this.loading = false;
    ...
    getApproveCount(){
        this.invoiceTakebackService.getApproveCount().then(
            res => this.count = res.item
        )
    }
    //点击li事件
    ...
    approveTakeback(invoiceStatus){
        ...
        let idStr = "";
        this.takebackList.filter(invoice => invoice.checked === true).forEach(invoice => {
            idStr += invoice.id+",";
        }); 
        this.invoiceTakebackService.approveTakeback(idStr.substring(0,idStr.length-1),invoiceStatus,remark)
            .then(res =>{
                if(res.success){
                    this.fullCheckedIndeterminate = false;
                    this.fullChecked = false;
    ...
    goDetail(id):void{ 
        if(this.flag=='0' && this.approveStatus=='2'){
            window.open("/invoice/takeback/apply/"+id);
        }else{
            window.open("/invoice/takeback/takebackDetail/"+id); 
        }
    }
}
