...
import { DbWfviewComponent, Pager, XcModalService, XcBaseModal, XcModalRef } from 'app/shared/index';
...
export class RevokedApproveComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        //流程按钮连接地址，组件未开发完
        this.appParms = {
            apiUrl_AR: environment_java.server + "/revoked/revoked-applys/reject",//拒绝
            apiUrl_Sign: environment_java.server + "/revoked/revoked-applys/flow-add-sign",//加签
            apiUrl_Transfer: environment_java.server + "/revoked/revoked-applys/flow-transfer",//转办
    ...
    searchInvoice(): void {
        ...
        var data1 = { "applyids": ids, "status": "1,2,3,5,6,8,9", "typeval": "CP", "sqruserItcode": this.revokedObj.revokedItcode };
    ...
    public saveBill(op) {
        ...
        var data = { "id": this.revokedId, "opinion": this.opinions, "apply": this.revokedObj };
        this.http.post(environment_java.server + '/revoked/revoked-applys/agreement', data, this.options)
            .toPromise()
            .then(res => {
                console.log(res);
                let rdata = res.json();
    ...
    load_invoice = function () {
        ...
        this.http.get(environment_java.server + '/revoked/find-revoked-applys', { search: params })
            .map(res => res.json())
            .subscribe(res => {
                console.log(res);
                if (res.item) {
                    ...
                    this.http.get(environment_java.server + 'common/sys-people/' + this.revokedObj.revokedItcode, {})
                        .map(res => res.json())
                        .subscribe(data => {
