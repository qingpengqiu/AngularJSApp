...
export class TakebackApproveComponent implements OnInit {
    ...
    //查询当前审批单
    load_invoice(){
        this.invoiceTakebackService.getTakebackInfo(this.takebackId)
            .then(res => {
                if (res.item) {
    ...
    if(invoiceStatus =="no" && this.takebackObj.approveNote==""){
        this.windowService.alert({ message: "请填写审批原因", type: 'warn' });
        return;
    }
    if(this.takebackObj.approveNote==""){
        this.takebackObj.approveNote = "同意";
    }
    this.windowService.confirm({message:'确定此操作吗？'}).subscribe(r =>{
        if(r){
            this.invoiceTakebackService.approveTakeback(this.takebackObj.id,invoiceStatus,this.takebackObj.approveNote)
                .then(res =>{
                    if(res.success){
                        this.clickFlag='1'; 
