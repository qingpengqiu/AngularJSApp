...
export class InvoiceApproveListComponent implements OnInit {
    ...
    ngOnInit() {
      ...
      //在初始化的时候 创建模型
      //编辑组件
      this.editForm = this.xcModalService.createModal(InvoiceApproveEditComponent);
     //查看组件
      this.detailForm = this.xcModalService.createModal(InvoiceDetailComponent);
     //模型关闭的时候 如果有改动，请求刷新
    ...
    approve(f){
        let idStr = "";
        this.invoiceList.filter(invoice => invoice.checked === true).forEach(invoice => {
            idStr += invoice.id+",";
        });
    ...
    approve2(f){
        this.invoiceList.filter(invoice => invoice.checked === true).forEach(invoice => {
            ...
            if(invoice.invoiceStatus==5 || invoice.invoiceStatus==6){
                if(f==1){//商务人员取走
                    this.invoiceApproveService.approveInvoice(invoice.id,'6');
                }
                if(f==2){//申请人员取走
                    this.invoiceApproveService.approveInvoice(invoice.id,'7');
                }
            } 
            //退票取走
            if(invoice.invoiceStatus==8 || invoice.invoiceStatus==9){
                if(f==1){//商务人员取走
                    this.invoiceApproveService.approveInvoice(invoice.id,'9');
                    
                }
                if(f==2){//申请人员取走
                    this.invoiceApproveService.approveInvoice(invoice.id,'10');
                }
            }
            ...
            if(r){
                 this.invoiceApproveService.approveInvoice(id,invoiceStatus)
                                .then(res =>{
                                    if(res.success){ 
                                        this.getApproveList();
                                        if(this.query.invoiceStatus=="1" || this.query.invoiceStatus =="2" || this.query.invoiceStatus =="3"|| this.query.invoiceStatus =="4"|| this.query.invoiceStatus =="5,8"){
                                            this.getPayeeCountList();
                                        }
                                        this.windowService.alert({message:res.message,type:"success"});
                                    }else{
