...
export class InvoiceApproveEditComponent implements OnInit{
  ...
  ngOnInit(){
    ...
    //客户查询模型关闭时
        ...
        this.debtForm.onHide().subscribe((data) => {
            if (data) {
                this.invoiceInfo.debtAmount = data.AMOUNT;
            }
        })
  }
  ...
  */
  toQueryPayee(){
     ...
     */
    toQueryDept(){
        if(this.invoiceInfo.customCode=="" || this.invoiceInfo.customCode== undefined){
            this.windowService.alert({ message: "请先选择客户", type: 'warn' });
            return;
        }
        if(this.invoiceInfo.payee=="" || this.invoiceInfo.payee== undefined){
            this.windowService.alert({ message: "请先选择收款人", type: 'warn' });
            return;
        }
        this.debtForm.show(0+"|"+this.invoiceInfo.customCode+"|"+this.invoiceInfo.payee);
    }
  ...
  //更新数据
  updateInvoice(){ 
        if(this.invoiceInfo.ticketNum===undefined){//必填验证
            this.windowService.alert({ message: "支票号不能为空", type: 'warn' });
        }else if(this.invoiceInfo.checkoutDate===undefined){
            this.windowService.alert({ message: "出票日期不能为空", type: 'warn' });
        }else if(this.invoiceInfo.ticketAmount===undefined){
            this.windowService.alert({ message: "支票金额不能为空", type: 'warn' });
        }else if(!this.checkNumber(this.invoiceInfo.ticketAmount)){
            this.windowService.alert({ message: "支票金额必须为数字，可保留两位小数", type: 'warn' });
        }else if(this.invoiceInfo.ticketAmount==0){
            this.windowService.alert({ message: "支票金额不能为0", type: 'warn' });
        }else if(this.invoiceInfo.checkoutAccount===undefined){
            this.windowService.alert({ message: "出票人账号不能为空", type: 'warn' });
        }else if(this.invoiceInfo.checkoutBank===undefined){
            this.windowService.alert({ message: "出票人银行不能为空", type: 'warn' });
        }else if(this.invoiceInfo.payee===undefined){
            this.windowService.alert({ message: "收款人不能为空", type: 'warn' });
        }else if(this.invoiceInfo.customCode===undefined){
            this.windowService.alert({ message: "客户代码不能为空", type: 'warn' });
        }else if(this.invoiceInfo.customName===undefined){
            this.windowService.alert({ message: "客户名称不能为空", type: 'warn' });
        }else if(this.invoiceInfo.contractNum===undefined){
            this.windowService.alert({ message: "合同编号不能为空", type: 'warn' });
        }else if(this.invoiceInfo.debtAmount !=undefined && !this.checkNumber(this.invoiceInfo.debtAmount)){
            this.windowService.alert({ message: "欠款金额必须为数字，可保留两位小数", type: 'warn' });
        }else if(!this.checkTheDate(this.invoiceInfo.checkoutDate)){
            this.windowService.alert({ message: "出票日期不能小于9天前", type: 'warn' });
        }else{
            this.subimtFlag = false;
            this.invoiceApproveService.updateInvoice(this.invoiceInfo)
            .then(data =>{
                            if(data.success){
