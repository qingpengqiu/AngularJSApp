...
import { Query, UserInfo, TradeTicketInfo } from "../../apply/invoice/invoice-info";
...
export class TradeTicketApproveListComponent implements OnInit {
    ...
    approve(f){
        ...
        let idStr = "";
        this.tradeticketList.filter(tradeTicket => tradeTicket.checked === true).forEach(tradeTicket => {
            idStr += tradeTicket.id+",";
        });
        this.tradeTicketService.approveTradeticket(idStr.substring(0,idStr.length-1),f)
                                .then(res =>{
                                    if(res.success){
                                        this.fullCheckedIndeterminate = false;
                                        this.fullChecked = false;
    ...
    approve2(f){
        ...
        this.tradeticketList.filter(tradeTicket => tradeTicket.checked === true).forEach(tradeTicket => {
            //拒收取走
            if(tradeTicket.tradeStatus==5 || tradeTicket.tradeStatus==6){
                if(f==1){//商务人员取走
                    this.tradeTicketService.approveTradeticket(tradeTicket.id,'6')
                        .then(res =>{
                                        if(res.success){
                                            this.fullCheckedIndeterminate = false;
                                            this.fullChecked = false;
                ...
                if(f==2){//申请人员取走
                    this.tradeTicketService.approveTradeticket(tradeTicket.id,'7')
                        .then(res =>{
                                    if(res.success){
                                        this.fullCheckedIndeterminate = false;
                                        this.fullChecked = false;
            ...
            //退票取走
            if(tradeTicket.tradeStatus==8 || tradeTicket.tradeStatus==9){
                if(f==1){//商务人员取走
                    this.tradeTicketService.approveTradeticket(tradeTicket.id,'9')
                        .then(res =>{
                                    if(res.success){
                                        this.fullCheckedIndeterminate = false;
                                        this.fullChecked = false;
                ...
                if(f==2){//申请人员取走
                    this.tradeTicketService.approveTradeticket(tradeTicket.id,'10')
                        .then(res =>{
                                    if(res.success){
                                        this.fullCheckedIndeterminate = false;
                                        this.fullChecked = false;
    ...
    refuse(id:string,tradeStatus:string){
        ...
        this.windowService.confirm({message:confirmStr}).subscribe(r =>{
            if(r){
                 this.tradeTicketService.approveTradeticket(id,tradeStatus)
                                .then(res =>{
                                    if(res.success){ 
                                        this.getApproveList();
                                        this.getPayeeCountList();
    ...
    getPayeeCountList(){
       ...
       this.tradeTicketService.getPayeeCountList(this.query)
                                 .then(resp => {
     ...
     */
    getBusinessList(){
        this.tradeTicketService.getBusinessList().then(
            resp =>  
            {
    ...
    goDetail(id):void{ 
        if(this.query.tradeStatus=='0'){//编辑
