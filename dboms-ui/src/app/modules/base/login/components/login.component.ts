...
export class LoginComponent implements OnInit {
  ...
  login() {
    ...
    this.authenticationService.login(this.userInfo.username, this.userInfo.password)
      ...
      data => {
        if (data.Result) {
          ...
          //2跳转
          if (this.returnUrl.indexOf("?") == -1) {//无参数 url
      ...
      //loginjava
      this.authenticationService.loginjava(this.userInfo.username)
        .subscribe(
        data => {
        },
        error => {
    ...
    //保存用户信息
    this.authenticationService.getUserInfoByItCode(this.userInfo.username).subscribe(
      data => {
        if (data.Result) {
