...
@Component({
    ...
    styleUrls: ['prepareApply-submit-sell.component.scss','./../../../scss/procurement.scss']
})
export class PrepareApplySubmitSellComponent implements OnInit,OnChanges {
    ...
    ngOnInit() {
        if(!this.purchaseRequisitioIid){//新建
            //新建时 加载本地选择的合同列表
    ...
    createInitialListData(state){//创建 tablist 展示列表 存储列表 的初始数据
        ...
        if(this.contractList && this.contractList.length){
            for(let i=0,len=this.contractList.length;i<len;i++){
                ...
                this.showList[i]["salecontractcode"]=this.contractList[i]["SC_Code"];
                url = "PurchaseManage/GetContractDetail/" + this.contractList[i]["SC_Code"];
    ...
    getTaxIncluSiveMoney(e,index,isInput) {//计算 含税总金额
        if (typeof(this.rate) == "undefined" || this.rate == null) {
            ...
            this.saleListSave[index].excludetaxmoney=Number(e).toFixed(2);//显示加.00
