...
@Component({
    ...
    styleUrls: ['prepareApply-submit-list.component.scss', './../../../scss/procurement.scss']
})
export class PrepareApplySubmitListComponent implements OnInit,OnChanges {
    ...
    fileUploadApi = "api/PurchaseManage/UploadPurchaseRequisitionDetails";//批量上传路径
    ...
    ngDoCheck() {
        ...
        if (JSON.stringify(this.contractList) != window.localStorage.getItem("prepareContractList")) {//合同列表变化
    ...
    getMaterialData(index, id) {//根据物料号读取信息
        if (id=="" || this._purchaseData.procurementList[index]["isExcel"]) {//如果是excel中读取，不更新物料描述
            ...
            this.getMaterial(id).then(response => {
