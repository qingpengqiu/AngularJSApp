...
export class ProcurementApplyMyApprovalComponent implements OnInit {
  ...
  routerJump(type,id,RequisitionType){//点击列表跳转的判断
    if(RequisitionType.indexOf("合同")!=-1){//合同采购申请
      if(type=='0'){
        let param=id.substring(id.indexOf("?"));
        window.open(dbomsPath+'procurement/approval-contractapply'+ param);
      }else{
        window.open(dbomsPath+'procurement/view-contractApply/'+ id);
      }
      return;
    }
    if(RequisitionType.indexOf("预下")!=-1){//预下采购申请
      if(type=='0'){
        let param=id.substring(id.indexOf("?"));
    ...
    if(RequisitionType.indexOf("备货")!=-1){//备货采购申请
      if(type=='0'){
        let param=id.substring(id.indexOf("?"));
