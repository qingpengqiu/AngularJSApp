...
@Component({
    ...
    styleUrls: ['new-contractApply.scss','./../../../../scss/procurement.scss']
})
export class NewContractApplyComponent implements OnInit {
    @Output() newContractShow = new EventEmitter;
    ...
    ngOnInit() {
        ...
        this.rank = new Rank();
        this.selectedList = JSON.parse(window.localStorage.getItem("contractList"));
    }
    clearSearch() {//重置搜索
        this.query = new Query();
    }
    ...
    initData(query: Query) {
        ...
        this.contractListService.getContractList(query).then(data => {
            ...
            //新增时添加默认
            let oldContractList = JSON.parse(window.localStorage.getItem("contractList"));
            for (let i = 0, len = this.contractList.length; i < len; i++) {//检查每个的checked
                for (let n = 0, len = oldContractList.length; n < len; n++) {
                    if (oldContractList[n].SC_Code == this.contractList[i].SC_Code) {
                        this.contractList[i]["checked"] = true;
                    }
                }
            }
    ...
    nextStep() {//下一步点击
        ...
        this.newContractShow.emit(false);
    }
    public closeConpoment(){
        this.newContractShow.emit(false);
    }
}