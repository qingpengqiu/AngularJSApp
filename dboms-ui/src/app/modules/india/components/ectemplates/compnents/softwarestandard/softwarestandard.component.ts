...
export class SoftwareStandardComponent implements OnInit {
  ...
  onSaveHandle() {
    ...
    let tempDate =  new Date(baseData.TakeDeliveryTime);
    let yearTakeDeliveryTime = tempDate.getFullYear();
    let monthTakeDeliveryTime = tempDate.getMonth() + 1;
    let dayTakeDeliveryTime = tempDate.getDate();
    baseData.TakeDeliveryTime = yearTakeDeliveryTime + "-" + monthTakeDeliveryTime + "-" + dayTakeDeliveryTime;
  ...
  onGetEContract() {
    ...
    this.http.get(url)
      ...
      data => {
        ...
        //争议解决方式
        if (this.formData.BaseData.DisputeDealtType === "0001") {
          $('#dispute0001').attr('checked', 'checked');
        } else if (this.formData.BaseData.DisputeDealtType === "001") {
          $('#dispute001').attr('checked', 'checked');
        } else if (this.formData.BaseData.DisputeDealtType === "01") {
  ...
  initDisputeDealtType(){
    if (this.formData.BaseData.DisputeDealtType === "0001") {
      $('#dispute0001').removeAttr('checked');
    } else if (this.formData.BaseData.DisputeDealtType === "001") {
      $('#dispute001').attr('checked');
    } else if (this.formData.BaseData.DisputeDealtType === "01") {
      $('#dispute01').attr('checked');
    }
