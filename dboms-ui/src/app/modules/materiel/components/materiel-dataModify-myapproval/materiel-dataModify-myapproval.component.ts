...
export class MaterielDataModifyMyApprovalComponent implements OnInit {
  ...
  onTab(e){//切换选项（待我审批，我已审批，全部）
   ...
   getDetail(ID,TaskId,e){//查看详情
    // let trID=e.target.parentNode.getAttribute("id");//获取点击的行的id
   
    // let TaskApproveResult= document.querySelector(`#${trID} td:last-child`).innerHTML;//获取点击行的最后一个元素里的内容
    // //let aa=$(`#${trID} td:last`).html();

    // if(TaskApproveResult=="驳回"){
    //   this.windowService.alert({message:"已驳回，请等待申请人提交",type:"fail"});
    // }else{
      
      
    // }
    this.router.params.subscribe(params=>{//获取路由传过来的值
      window.open(dbomsPath+"mate/edit-data/"+params.id+ID+"?"+TaskId);
    });
    //console.log(TaskId);
