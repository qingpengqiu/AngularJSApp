...
export class EditMaterielNewReturnService implements OnInit {
            ...
            this.returnService.saveCommonlyMaterial(this.reqData).then(data => {
                ...
                if (!data.success) {//弹出错误提示
                    ...
                    if (this.reqData.MaterialRecordID == null) {//判断MaterialMasterID的值，提示是新建还是保存
                        ...
                        this.windowService.alert({ message: "返款申请保存成功", type: "success" });
                        this.reqData.MaterialRecordID=data.data.data.MaterialRecordID;//如果保存成功，则写入返回的id
                    
                    } else {
                        this.windowService.alert({ message: "返款申请更新成功", type: "success" });
                    }
                ...
                this.returnService.saveCommonlyMaterial(this.reqData).then(data => {
                    ...
                    if (!data.success) {//弹出错误提示
                        ...
                        this.windowService.alert({message:"物料号生成成功",type:"success"}).subscribe(()=>{
                            //this.reqData.MaterialRecordID = data.data.data.MaterialRecordID;//如果保存成功，则写入返回的id
                            this.routerLink.navigate(['/mate/edit-srs',this.reqData.MaterialRecordID]);//跳转到详情页
                        });
    ...
    selectTemplate(templateId){//选择物料模板
        ...
        this.returnService.seeTemplate({ TemplateID: templateId }).then(data => {//选择是请求数据库，查询以建好的模板
            ...
            this.allFactory = [data.data.list[0].Factory,data.data.list[0].FactoryName];
            this.allSerialNumParameter = [data.data.list[0].SerialNumParameter,data.data.list[0].SerialNumParameterName];
            this.allMaterialType = [data.data.list[0].MaterialType,data.data.list[0].MaterialTypeName];
            this.allTaxType = [data.data.list[0].TaxTypeID,data.data.list[0].TaxTypeName];
            this.allMaterialGroup = [data.data.list[0].MaterialGroup,data.data.list[0].MaterialGroupName];
            this.allBaseUnitOfMeasure = [data.data.list[0].BaseUnitOfMeasure,data.data.list[0].BaseUnitOfMeasureName];
            this.allBrand = [data.data.list[0].Brand,data.data.list[0].BrandName];
            this.allSupplierCodeSAP = [data.data.list[0].SupplierCodeSAP,data.data.list[0].SupplierName];
    ...
    ngOnInit() {
       ...
       this.router.params.subscribe(params=>{
        ...
        if(params.id!=0){
          ...
          this.allFactory=[this.reqData.Factory,this.reqData.FactoryName];
          this.allSerialNumParameter=[this.reqData.SerialNumParameter,this.reqData.SerialNumParameterName];
          this.allMaterialType=[this.reqData.MaterialType,this.reqData.MaterialTypeName];
          this.allSupplierCodeSAP=[this.reqData.SupplierCodeSAP,this.reqData.SupplierName];
          this.allMaterialGroup=[this.reqData.MaterialGroup,this.reqData.MaterialGroupName];
          this.allBrand=[this.reqData.Brand,this.reqData.BrandName];
          this.allBaseUnitOfMeasure=[this.reqData.BaseUnitOfMeasure,this.reqData.BaseUnitOfMeasureName];
          this.allAvailabilityChecking=[this.reqData.AvailabilityChecking,this.reqData.AvailabilityCheckingName];
          this.allSubjectSettingGroup=[this.reqData.SubjectSettingGroup,this.reqData.SubjectSettingGroupName];
          this.allTaxClassifications=[this.reqData.TaxClassifications,this.reqData.TaxClassificationsName];
          this.allTaxType=[this.reqData.TaxTypeID,this.reqData.TaxTypeName];
