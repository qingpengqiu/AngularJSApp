            ...
            this.editTitle="编辑一般物料申请"
             ...
             this.commonlyMaterielService.editMateriel(this.reqEditMateriel).then(data => {
              ...
              //  console.log(JSON.stringify(this.reqData));
               ...
               //转换标示
               if(this.reqData.IsBatchManage!=0){
                   this.reqData.IsBatchManage=true;
               }else{
                   this.reqData.IsBatchManage=false;
               }
...
submit(){//生成物料编号
  ...
  save(InfoStatus) {
    ...
    this.reqData.SupplierCodeSAP=this.allSupplierCodeSAP[0]?this.allSupplierCodeSAP[0]:"";
    this.reqData.SupplierName=this.allSupplierCodeSAP[1]?this.allSupplierCodeSAP[1]:"";
    ...
    //验证所有物料模版的必填项是否为空
    let isMustFormIsFalse=this.reqData.Factory==""||this.reqData.SerialNumParameter==""||this.reqData.MaterialType==""||this.reqData.SupplierCodeSAP==""||this.reqData.MaterialGroup==""||this.reqData.Brand==""||this.reqData.BaseUnitOfMeasure==""||this.reqData.AvailabilityChecking==""||this.reqData.SubjectSettingGroup==""||this.reqData.TaxClassifications=="";
      ...
      this.commonlyMaterielService.saveCommonlyMaterial(this.reqData).then(data => {
        ...
        if (!data.success) {//弹出错误提示
          ...
          if (this.reqData.MaterialRecordID == null) {//判断MaterialMasterID的值，提示是新建还是保存
            ...
            this.windowService.alert({ message: "一般物料申请保存成功", type: "success" });
  ...
  SupplierCodeSAPVerify(){//验证供应商物料编号如果为40开头，则可用性检查为02，否则为04
    
    this.unSelect=false;
    if(this.allSupplierCodeSAP[0].substring(0,2)==="40"){
