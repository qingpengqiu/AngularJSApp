...
import { ReqData, ReqSearchData, ReqDeleteList, ReqEditMateriel, CommonlyMaterielAndReturnService, ExtensionListDate } from "../../services/materiel-commonlyMateriel&return.service";
...
@Component({
    ...
    styleUrls: ['materiel-commonly.component.scss', '../../scss/materiel.component.scss']
})
...
export class MaterielCommonlyComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        let foo = () => { this.initData(this.reqSearchData) };
  ...
  deleteList(param: any) {
      ...
      if (typeof param == "string") {//删除单条数据
          this.windowService.confirm({ message: "确定删除？" }).subscribe({
              next: (v) => {
                  if (v) {
                      if (this.reqData.InfoStatus != 1) {//已生成物料编号的，将不可删除
                          this.commonlyMaterielService.deleteListElement({ "MaterialRecordID": param, "ApplyType": 1 }).then(callback);
                      }
                  }
              }
          });
          ...
          this.windowService.confirm({ message: `确定删除您选中的${this.checkedNum}项？` }).subscribe(v => {
              if (v) {
                  ...
                  param.filter(item => item.checked === true).forEach(item => {
                      ObList.push(this.commonlyMaterielService.deleteListElement({ "MaterialRecordID": item.MaterialRecordID, "ApplyType": 1 }));
                  });
  ...
  initData(reqSearchData: ReqSearchData) {
    ...
    extendMateriel(mID, mERP) {
        if (!!mID && !!mERP) {//如果传入的物料编号和主键ID都不为空
            this.extensionListDate.MaterialRecordId = mID;
            this.extensionListDate.MaterialERPCode = mERP;
            this.loading=true;//显示loading等待画面
            this.commonlyMaterielService.extension(this.extensionListDate).then(data => {

                if(data.success){
                    this.loading=false;//隐藏等待画面                    
                    this.windowService.alert({message:"扩展物料成功",type:"success"}).subscribe(()=>{//如果扩展成功，请求一次接口，刷新列表页面
            ...
            this.windowService.alert({ message: "只能扩展已生成物料ERP编号的物料", type: "fail" });
        }
    }
