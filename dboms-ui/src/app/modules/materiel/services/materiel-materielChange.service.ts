...
export class DetailExport{//转出物料数据对象
  Id:string;//导出行ID,
...
export class DetailImport{//转入物料数据对象
  Id:string;// 36,
  ChangeId:string;// 28,
...
export class MaterielChangeData {//物料变更基础数据
  Id:string;//主键ID
...
export class DelDataMaterielChange{//删除
  ID:string;//主键ID
}
...
export class SalseList{//通过物料号查询销售合同列表
...
export class BackSaleApproval{//审批组件内部调用参数
  taskid:string="";// "审批任务ID", 
  nodeid:string="";// "审批节点ID"
  instanceid:string;//实力Id
  vouncher:string="";//物料凭证号
...
export class GetBasedata{//获取审批数据字符串
  BizScopeCode:string="";//业务范围
  WFCategory:string="物料变更";//审批流程分类
}
...
export class MaterielChangeService {
  ...
  searchMaterielChange(searchData: any){
    ...
    return this.http.post(environment.server + "/materialchange/list", searchData,options)
                    .toPromise()
  ...
  deleteDataMaterielChange(delData: any){
    let headers = new Headers({ 'ticket': localStorage.getItem('ticket') });
    let options = new RequestOptions({ headers: headers });
    return this.http.post(environment.server + "/materialchange/del", delData,options)
                    .toPromise()
                    .then(response => response.json())
  }
  ...
  detailData(detailData: any){
    let headers = new Headers({ 'ticket': localStorage.getItem('ticket') });
    let options = new RequestOptions({ headers: headers });
    return this.http.post(environment.server + "/materialchange/detail", detailData,options)
                    .toPromise()
                    .then(response => response.json())
  }
  //提交和暂存
  saveData(saveData:any){
    let headers = new Headers({ 'ticket': localStorage.getItem('ticket') });
    let options = new RequestOptions({ headers: headers });
    return this.http.post(environment.server + "materialchange/add", saveData,options)
    .toPromise()
    .then(response => response.json())
  }
  //根据物料编号，查询相关的销售合同列表
  editSalesList(saleslist:any){
    ...
    let headers = new Headers({ 'ticket': localStorage.getItem('ticket') });
    let options = new RequestOptions({ headers: headers });
    return this.http.post(environment.server + "/materialchange/getsalecontractno", saleslist,options)
    .toPromise()
    .then(response => response.json())
  ...
  approvalList(approvalist:any){
    let headers = new Headers({ 'ticket': localStorage.getItem('ticket') });
    let options = new RequestOptions({ headers: headers });
    return this.http.post(environment.server + "/materialchange/approvelist", approvalist,options)
    .toPromise()
    .then(response => response.json())
  }
  //根据物料号获取物料名称
  getMaterielInformation(materielCodeFactoryData:any){
    let headers = new Headers({ 'ticket': localStorage.getItem('ticket') });
    let options = new RequestOptions({ headers: headers });
    return this.http.post(environment.server+"materialchange/getmaterialinfo", materielCodeFactoryData , options).toPromise().then(response => response.json());
  }
  //销售订单反填环节的审批
  backSaleApprove(approveData:BackSaleApproval){
    let headers = new Headers({ 'ticket': localStorage.getItem('ticket') });
    let options = new RequestOptions({ headers: headers });
    return this.http.post(environment.server + "/materialchange/approve", approveData,options)
    .toPromise()
    .then(response => response.json())
  }
  //根据ID获取全景图
  getPanoramagram(ID:string){//根据ID获取全景图
    let headers = new Headers({ 'ticket': localStorage.getItem('ticket') });
    let options = new RequestOptions({ headers: headers });
    return this.http.get(environment.server+"materialchange/GetPanoramagram/"+`${ID}`, options).toPromise().then(response => response.json());
  }
  //获取销售订单过来的物料信息
  getSaleMateriel(selaData:any){//获取销售订单过来的物料信息
    let headers = new Headers({ 'ticket': localStorage.getItem('ticket') });
    let options = new RequestOptions({ headers: headers });
    return this.http.post(environment.server+"materialchange/sponsor/", selaData,options).toPromise().then(response => response.json());
  ...
  getBasedata(basedata:any){
    let headers = new Headers({ 'ticket': localStorage.getItem('ticket') });
    let options = new RequestOptions({ headers: headers });
    return this.http.post(environment.server+"materialchange/basedata/", basedata,options).toPromise().then(response => response.json());
  ...
  getSaleImportMaterial(ChangeId:any){
    let headers = new Headers({ 'ticket': localStorage.getItem('ticket') });
    let options = new RequestOptions({ headers: headers });
    return this.http.get(environment.server+"materialchange/getsalesorders/"+`${ChangeId}`, options).toPromise().then(response => response.json());
  }
  //将销售订单反填的数据传给后台
  writeImportMaterial(importMaterial:any){
    let headers = new Headers({ 'ticket': localStorage.getItem('ticket') });
    let options = new RequestOptions({ headers: headers });
    return this.http.post(environment.server+"materialchange/writeimport", importMaterial,options).toPromise().then(response => response.json());
  ...
  isDeviate(materielData:any){
    let headers = new Headers({ 'ticket': localStorage.getItem('ticket') });
    let options = new RequestOptions({ headers: headers });
    return this.http.post(environment.server+"materialchange/isdeviate", materielData,options).toPromise().then(response => response.json());
