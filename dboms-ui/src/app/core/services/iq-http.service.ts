...
export class iqHttpService extends Http {
      ...
      return Observable.create((observer) => {
        observable.subscribe(res => {
          ...
          if(res.headers.get("pmcode") == 401){
            this.windowservice.alert({message:this.status['status.401'],type:"fail"});
          }
        }, (err) => {
  ...
  getRequestOptionArgs(options ? : RequestOptionsArgs): RequestOptionsArgs {
        ...
        if(window.localStorage.getItem('ticket')!=null){
          ...
          if(options.headers.get("ticket")==null){

            options.headers.append("ticket", window.localStorage.getItem('ticket'));
          }
